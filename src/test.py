...
from .util import get_model_dir, fast_intersection_and_union_for_pred, setup_seed, resume_random_state, find_free_port, setup, \
    cleanup, get_cfg

from .bcm import Classifier as bcm_Classifier
from .bcm import BaseClassifier
...
def validate(args: argparse.Namespace, val_loader: torch.utils.data.DataLoader, model: DDP) -> Tuple[torch.tensor, torch.tensor]:
    ...
    for run in range(args.n_runs):
        ...
        if args.method == "BCM":
            ...
            base_classifier = BaseClassifier(base_weight, base_bias)
            args.run = run
            tic = time.time()
            classifier = bcm_Classifier(args, base_classifier, n_tasks=1)
            classifier.init_table(features_s, gt_s)
            classifier.train(features_s)
            train_time += time.time() - tic

        for iter_cnt in tqdm(range(nb_episodes), leave=True):
            t0 = time.time()
            with torch.no_grad():
                try:
            ...
            elif args.method == "BCM":
                pred = classifier.predict(features_q, H, W)

            # intersection, union, target = fast_intersection_and_union(probas, gt_q)  # [batch_size_val, 1, num_classes]
            intersection, union, target = fast_intersection_and_union_for_pred(pred, gt_q, num_classes=classifier.num_classes)
        ...
        base_count, novel_count, sum_base_IoU, sum_novel_IoU = 4 * [0]
        for i, class_ in enumerate(val_loader.dataset.all_classes):
        #for i, class_ in enumerate(args.all_class_list):
            if cls_union[i] == 0:
                print(class_, ' is zero')
                continue
            IoU = cls_intersection[i] / (cls_union[i] + 1e-10)
            if class_ in val_loader.dataset.base_class_list:
                sum_base_IoU += IoU
